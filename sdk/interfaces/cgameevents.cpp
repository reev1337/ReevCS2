...
#include "../../features.h"
#include <thread>
#include "../../features/misc/BulletTracer.hh"
...
void CEvents::OnPaint()
{
	...
	if (flHurtTime + 1.2f >= curtime)
	{
		...
		if (D::WorldToScreen(pos, &vecScreen))
			D::pDrawListActive->AddText(FONT::pMain, 15, vecScreen, Color_t(230, 230, 230).GetU32(), std::to_string(this->damage).c_str());
...
void CEvents::OnBulletImpact(CGameEvent* event)
{
	...
	if (C_GET(bool, Vars.bDebugShootHitbox)) {
	...
	if (C_GET(bool, Vars.bBulletTraceEnemy))
	{
		if (!player->IsLocalPlayerController())
		{
			if (player->GetPlayerPawn() && player->GetPlayerPawn()->GetTeam() != pLocalPawn->GetTeam())
				F::MISC::BULLETTRACER::AddBulletTrace(player->GetPlayerPawn()->GetGameSceneNode()->GetAbsOrigin() + player->GetPlayerPawn()->GetViewOffset(), position, C_GET(Color_t, Vars.colBulletTraceEnemy));
		}
	}
	if (C_GET(bool, Vars.bBulletTraceLocal))
	{
		if (player->IsLocalPlayerController())
			F::MISC::BULLETTRACER::AddBulletTrace(pLocalPawn->GetGameSceneNode()->GetAbsOrigin() + pLocalPawn->GetViewOffset(), position, C_GET(Color_t, Vars.colBulletTraceLocal));
	}
