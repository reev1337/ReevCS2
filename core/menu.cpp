...
void MENU::RenderMainWindow()
{
	...
	if (ImGuiWindow* pMainWindow = ImGui::FindWindowByName(CS_XOR("HackvsHack.net")); pMainWindow != nullptr)
	{
...
if (settings_window)
	...
	ImGui::Begin(CS_XOR("Xenon"), &bMainWindowOpened, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoBackground);
	{
		...
		pWindowDrawList->AddText(FONT::pCheatLogo, 35.f * flDpiScale, ImVec2(20.000f * flDpiScale + p.x, 15.000f * flDpiScale + p.y), ImColor(0.60f, 0.60f, 0.60f, 0.70f), "HackvsHack.net");
		...
		if (ImGui::TabButton("N", "MissBot", ImVec2(190 * flDpiScale, 70 * flDpiScale)) && nCurrentMainTab != 0)
		{
			nCurrentMainTab = 0;
			active = true;
		}

		if (ImGui::TabButton("Q", "AntiHit", ImVec2(190 * flDpiScale, 70 * flDpiScale)) && nCurrentMainTab != 1)
		{
			nCurrentMainTab = 1;
			active = true;
		}

		if (ImGui::TabButton("P", "Legitbotik", ImVec2(190 * flDpiScale, 70 * flDpiScale)) && nCurrentMainTab != 2)
		{
			nCurrentMainTab = 2;
			active = true;
		}
		...
		if (ImGui::TabButton("O", "Movment", ImVec2(190 * flDpiScale, 70 * flDpiScale)) && nCurrentMainTab != 4)
		{
			nCurrentMainTab = 4;
			active = true;
		}
		...
		if (ImGui::TabButton("S", "CFG", ImVec2(190, 70)) && nCurrentMainTab != 8)
		{
			nCurrentMainTab = 8;
			active = true;
		}
...
void T::VisualsView()
{
	...
	ImGui::BeginChildSection(CS_XOR("Thirdperson"), ImVec2(339 * MENU::flDpiScale, 253 * MENU::flDpiScale), true);
	{
		...
		ImGui::ToggleButton(CS_XOR("Thirdperson spec"), &C_GET(bool, Vars.bThirdPersonInSpec));
...
void T::Miscellaneous()
{
	ImGui::BeginChildSection(CS_XOR("Misc General"), ImVec2(339 * MENU::flDpiScale, 253 * MENU::flDpiScale), true);
	{
		...
		ImGui::ToggleButton(CS_XOR("AntiUntrusted"), &C_GET(bool, Vars.bAntiUntrusted));
	...
	{
		ImGui::ToggleButton(CS_XOR("Bhop"), &C_GET(bool, Vars.bAutoBHop));
...
void T::MiscellaneousInventory()
{
	ImGui::BeginDisabled(true);
	{
		ImGui::Text(CS_XOR("Wait to update..."));
	}
	ImGui::EndDisabled();
}
