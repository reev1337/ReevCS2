...
bool H::Setup()
{
    ...
    if (!hkApplyViewPunch.Create(MEM::FindPattern(CLIENT_DLL, CS_XOR("40 55 53 56 41 56 41 57 48 8D 6C 24 ? 48 81 EC ? ? ? ? 48 8B D9")), reinterpret_cast<void*>(&ApplyViewPunch)))
		return false;
	L_PRINT(LOG_INFO) << CS_XOR("\"ApplyViewPunch\" hook has been created");


	if (!hkAllowCameraAngleChange.Create(MEM::FindPattern(CLIENT_DLL, CS_XOR("40 53 48 83 EC 20 48 8B D9 E8 ? ? ? ? C6 83 ? ? ? ? ? 33 C0 48 C7 83 ? ? ? ? ? ? ? ?")), reinterpret_cast<void*>(&AllowCameraAngleChange)))
		return false;
...
void CS_FASTCALL H::OverrideView(void* pClientModeCSNormal, CViewSetup* pSetup)
{
	...
	if (C_GET(bool, Vars.bAspectRatio)) {
		pSetup->nSomeFlags |= 2;
...
bool CS_FASTCALL H::EmitUiPanorama(__int64 a1, __int64 a2, const char* name, float a4)
{
	...
	if (FNV1A::Hash(name) == FNV1A::Hash("popup_accept_match_found"))
	{
		...
		static auto fn = reinterpret_cast<fnSetViewAngle>(MEM::FindPattern(CLIENT_DLL, CS_XOR("85 D2 75 3F 48")));
