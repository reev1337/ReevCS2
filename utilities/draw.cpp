...
bool ImGui::HotKey(const char* szLabel, KeyBind_t* pKeyBind, const bool bAllowSwitch)
{
	...
	if (bAllowSwitch)
	{
		// Выделяем память для уникального идентификатора
		const size_t length = CRT::StringLength(szLabel) + 6;
		char* szUniqueID = static_cast<char*>(MEM_STACKALLOC(length));
		CRT::StringCopy(szUniqueID, CS_XOR("key##"));
		CRT::StringCat(szUniqueID, szLabel);
		...
		if (BeginPopup(szUniqueID))
		{
			...
			Text("Mode: %s", pKeyBind->nMode == EKeyBindMode::HOLD ? "Hold" : "Toggle");
			Separator();

			// Кнопки для выбора режима с подсветкой активного
			PushStyleColor(ImGuiCol_Button, pKeyBind->nMode == EKeyBindMode::HOLD ? ImVec4(0.26f, 0.59f, 0.98f, 0.40f) : ImVec4(0.20f, 0.20f, 0.20f, 1.00f));
			if (Button("Hold"))
			{
				pKeyBind->nMode = EKeyBindMode::HOLD;
				CloseCurrentPopup();
			}
			PopStyleColor();
			...
			PushStyleColor(ImGuiCol_Button, pKeyBind->nMode == EKeyBindMode::TOGGLE ? ImVec4(0.26f, 0.59f, 0.98f, 0.40f) : ImVec4(0.20f, 0.20f, 0.20f, 1.00f));
			if (Button("Toggle"))
			{
				pKeyBind->nMode = EKeyBindMode::TOGGLE;
				CloseCurrentPopup();
			}
			PopStyleColor();
...
#pragma endregion
	...
	FONT::pEspPName = io.Fonts->AddFontFromFileTTF(CS_XOR("C:\\Windows\\Fonts\\Verdana.ttf"), 10.f, &imVerdanaConfig, io.Fonts->GetGlyphRangesCyrillic());

	FONT::pEspName = io.Fonts->AddFontFromFileTTF(CS_XOR("C:\\Windows\\Fonts\\Verdana.ttf"), 10.f, &imVerdanaConfig, io.Fonts->GetGlyphRangesDefault());

	FONT::pEspFlagsName = io.Fonts->AddFontFromFileTTF(CS_XOR("C:\\Windows\\Fonts\\Verdana.ttf"), 10.f, &imVerdanaConfig, io.Fonts->GetGlyphRangesDefault());
