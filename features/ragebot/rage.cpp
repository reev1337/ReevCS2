...
{
	if (!I::GlobalVars || !I::NetworkClientService || !I::Cvar)
		return FLT_MAX;
...
struct record
{
	...
	void store(C_CSPlayerPawn* pawn)
	{
		...
		static auto calc_world_space_bone = reinterpret_cast<void(__fastcall*)(void*, uint32_t)>(MEM::FindPattern(CLIENT_DLL, "40 55 56 57 41 54 41 55 41 56 41 57 48 81 EC D0"));
		calc_world_space_bone(skeleton_instance, FLAG_ALL_BONE_FLAGS);
...
{
	if (!entity || !hitgroup || !m_data.m_wpn_data)
		return;
	...
	const bool is_ct = entity->GetLocalPlayer()->GetTeam() == 3;
	float head_damage_scale = is_ct ? damage_scale_ct_head : damage_scale_t_head;
	const float body_damage_scale = is_ct ? damage_scale_ct_body : damage_scale_t_body;

	auto pWeaponServices = entity->GetItemServices();
	if (pWeaponServices && pWeaponServices->HasHeavyArmor())
...
__forceinline static bool hitchance(C_CSPlayerPawn* pPawn, C_CSWeaponBase* pWeapon, Vector_t vecTarget)
{
	if (!pPawn || !SDK::LocalPawn || !pWeapon)
		return false;

	if (I::Cvar->Find(FNV1A::HashConst("weapon_accuracy_nospread"))->value.i1 || C_GET(bool, Vars.bNoSpread))
		return true;

	const Vector_t vecSource = SDK::LocalPawn->GetEyePosition();
	const int nWantedHitCount = 1024;
	...
	const float flRange = m_data.m_wpn_data->GetRange();
	const bool bIsRevolver = [&]()
	{
		if (auto attManager = pWeapon->GetAttributeManager())
			if (auto item = attManager->GetItem())
				return item->GetItemDefinitionIndex() == WEAPON_R8_REVOLVER;
		return false;
	}();
	...
	{
		const float flRand1 = I::RandomFloat(0.f, 1.f);
		const float flRand2 = I::RandomFloat(0.f, 2.f * _PI);
		const float flRand3 = I::RandomFloat(0.f, 1.f);
		const float flRand4 = I::RandomFloat(0.f, 2.f * _PI);
		...
		const float flInaccuracyX = cos(flRand2) * flWeaponInaccuracy;
		const float flInaccuracyY = sin(flRand2) * flWeaponInaccuracy;
		const float flSpreadX = cos(flRand4) * flWeaponSpread;
		const float flSpreadY = sin(flRand4) * flWeaponSpread;

		Vector_t vecSpreadView(flInaccuracyX + flSpreadX, flInaccuracyY + flSpreadY, 0);

		Vector_t vecDirection = vecForward + (vecRight * vecSpreadView.x) + (vecUp * vecSpreadView.y);
		vecDirection = vecDirection.Normalized();

		Vector_t vecViewForward = vecSource + (vecDirection * flRange);
		...
		const int nRemainingShots = nWantedHitCount - i - 1;
		if (nHits + nRemainingShots < nNeededHits)
			return false;
	}
	return nHits >= nNeededHits;
}
...
bool NoSpread(CUserCmd* pCmd, Vector_t bestPoint, C_CSPlayerPawn* player, int index)
{
	...
	Vector_t vecDir = bestPoint - m_data.m_local->GetEyePosition();
	float flDist = vecDir.NormalizeInPlace();

	QAngle_t angSpreadCorrection;
	VectorAngles(vecDir, angSpreadCorrection);

	pCmd->csgoUserCmd.pBaseCmd->pViewAngles->angValue = angSpreadCorrection;
	...
	Vector_t vecEnd = m_data.m_local->GetEyePosition() + vecDir * range;
	I::GameTraceManager->TraceShape(&ray, m_data.m_local->GetEyePosition(), vecEnd, &filter, &trace);
	I::GameTraceManager->ClipRayToEntity(&ray, m_data.m_local->GetEyePosition(), vecEnd, player, &filter, &trace);

	return trace.m_pHitEntity && trace.m_pHitEntity->GetRefEHandle().GetEntryIndex() == player->GetRefEHandle().GetEntryIndex();
}
...
bool CanShoot(CCSPlayerController* pLocalController, C_CSWeaponBaseGun* active_weapon, int tick)
{
	if (!active_weapon || !active_weapon->GetClip1())
		return false;
	...
	if (active_weapon->GetAttributeManager()->GetItem()->GetItemDefinitionIndex() == WEAPON_DESERT_EAGLE)
	{
		if (active_weapon->GetPostponeFireReadyTicks() > tick)
			return false;
	}

	if (active_weapon->GetAttributeManager()->GetItem()->GetItemDefinitionIndex() == WEAPON_GLOCK_18)
	{
		if (active_weapon->GetPostponeFireReadyTicks() > tick)
			return false;
	}

	auto weapon_data = active_weapon->GetWeaponVData();
	if (!weapon_data)
		return false;

	if (weapon_data->GetWeaponType() == WEAPONTYPE_SNIPER_RIFLE &&
	C_GET(bool, Vars.bAutoScope) &&
	active_weapon->GetZoomLevel() == 0)
	{
		return false;
	}
...
void SetAdaptiveWeapon(C_CSWeaponBase* weapon)
{
	...
	int weaponType = weapon->GetWeaponVData()->GetWeaponType();
	int profile = 0; // Default
	...
	if (has_awp)
		...
		profile = 1;
	else if (weaponType == WEAPONTYPE_MACHINEGUN || weaponType == WEAPONTYPE_RIFLE)
		profile = 3;
	else if (weaponType == WEAPONTYPE_SNIPER_RIFLE)
		profile = 4;

	m_data.hitChance = C_GET_ARRAY(int, 7, Vars.iHitChance, profile);
	m_data.hitboxes = C_GET_ARRAY(unsigned int, 7, Vars.bRageHitboxes, profile);
	m_data.multipoint = C_GET_ARRAY(bool, 7, Vars.bMultiPoint, profile);
	m_data.penetrationCount = C_GET_ARRAY(int, 7, Vars.iPenetrationCount, profile);
	m_data.minDamage = C_GET_ARRAY(int, 7, Vars.iMinDamage, profile);
	m_data.overrideDamage = C_GET_ARRAY(int, 7, Vars.iOverrideDamage, profile);
}

void F::RAGEBOT::AutoPeek(CUserCmd* pCmd, CBaseUserCmdPB* pUserCmd, C_CSPlayerPawn* pLocalPawn)
{
	// Single implementation combining any needed functionality from both versions
	if (!C_GET(bool, Vars.bAutoPeek))
	{
		quickpeek_retract = false;
		quickpeek_retract_shoot = false;
	...
	bool isMovementKeyPressed = (pCmd->nButtons.nValue & IN_FORWARD) || (pCmd->nButtons.nValue & IN_BACK) ||
	(pCmd->nButtons.nValue & IN_MOVELEFT) || (pCmd->nButtons.nValue & IN_MOVERIGHT);

	quickpeek_retract = !isMovementKeyPressed;
	...
	if ((quickpeek_retract || quickpeek_retract_shoot) && !retractPos.IsZero())
	{
		...
		QAngle_t angle = CalcAngles(pLocalPawn->GetGameSceneNode()->GetAbsOrigin(), retractPos);
		const float delta = remainderf(angle.y - pUserCmd->pViewAngles->angValue.y, 360.f);
		const float yaw = DEG2RAD(delta);

		Vector_t move(1.f, 0.f, 0.f);
		Vector_t move_backup = move;

		move.x = move_backup.x * std::cos(yaw) - move_backup.y * std::sin(yaw);
		move.y = move_backup.x * std::sin(yaw) + move_backup.y * std::cos(yaw);

		float mul = 1.f / std::max(std::fabsf(move.x), std::fabsf(move.y));
		move.x *= mul;
		move.y *= mul;

		static float trigger_dist = 10.f;
		float mul2 = dist < trigger_dist ? dist / trigger_dist : 1.f;
...
{
	if (!C_GET(bool, Vars.bAutoStop) || !(pLocalPawn->GetFlags() & FL_ONGROUND))
		return;
	...
	float max_speed = active_weapon->get_max_speed();
	float stop_threshold = max_speed * 0.33f;
...
void backtrack_render()
{
	if (!C_GET(bool, Vars.bBackTrackRender))
		return;
...
{
	if (!rec || !rec->m_skeleton || !rec->m_skeleton->pBoneCache)
		return false;

	int min_damage = C_GET(bool, Vars.bEnableOverrideDamage) ?
	std::min(m_data.overrideDamage, entity->m_pawn->GetHealth()) :
	std::min(m_data.minDamage, entity->m_pawn->GetHealth() + 30);
	...
	for (int hitbox : m_hitboxes)
	{
		...
		MultiPoints(rec->m_skeleton->pBoneCache, hitbox, points, nullptr);
	...
	return best_damage >= min_damage;
}

void AutoScope(CUserCmd* pCmd, C_CSWeaponBaseGun* active_weapon_base_gun)
{
	//if (active_weapon_base_gun->GetZoomLevel() == 0 && C_GET(bool, Vars.bAutoScope))
	//{
	//	if (!(pCmd->nButtons.nValue & ECommandButtons::IN_ATTACK))
	//	{
	//		int weaponIndex = active_weapon_base_gun->GetAttributeManager()->GetItem()->GetItemDefinitionIndex();
	//		if (weaponIndex == WEAPON_AWP || weaponIndex == WEAPON_SSG_08)
	//		{
	//			pCmd->nButtons.nValue |= ECommandButtons::IN_SECOND_ATTACK; // Активируем автоскоп
	//		}
	//	}
	//}
}
...
{
	if (!C_GET(bool, Vars.bRageBot) || !pLocalPawn || !pLocalPawn->IsValidMoveType())
	{
		quickpeek_retract = false;
		quickpeek_retract_shoot = false;
		return;
	...
	if (!active_weapon || !pWeaponVData || pWeaponVData->GetWeaponType() == WEAPONTYPE_KNIFE || pWeaponVData->GetWeaponType() == WEAPONTYPE_GRENADE)
		return;
	...
	std::vector<TargetInfo_t> valid_targets;
	...
	for (auto& entity : entity_list)
	{
		...
		if (!pPlayer || pPlayer == pLocalController || !pPlayer->IsPawnAlive())
			continue;

		C_CSPlayerPawn* pPawn = I::GameResourceService->pGameEntitySystem->Get<C_CSPlayerPawn>(pPlayer->GetPawnHandle());
		if (!pPawn || !pLocalPawn->IsOtherEnemy(pPawn) || pPawn->GetGunGameImmunity())
			continue;
		...
		if (records.find(pPlayer->GetPawnHandle().ToInt()) == records.end())
			records[pPlayer->GetPawnHandle().ToInt()] = std::make_unique<backtrack_entity>(pPawn);
		...
		try
		{
			...
			const auto best_records = entity_record->find_best_records(las_valid_sim_time);
			if (!best_records.record1 || !best_records.record2)
				continue;
			...
			if (FireTarget(entity_record, best_records.record1, bestDamage, bestPoint))
				bestRecord = best_records.record1;
			else if (FireTarget(entity_record, best_records.record2, bestDamage, bestPoint))
				bestRecord = best_records.record2;

			if (bestRecord)
				valid_targets.push_back({ pPlayer->GetPawnHandle().ToInt(), bestDamage, distance, bestPoint, bestRecord });
		}
		catch (...)
		{ }
	}

	std::sort(valid_targets.begin(), valid_targets.end(), [](const TargetInfo_t& a, const TargetInfo_t& b)
	{
	...
	const auto& best_target = valid_targets.front();
	auto entity = records[best_target.index].get();
	if (!entity || !entity->m_pawn)
		return;

	if (!active_weapon_base_gun->CanPrimaryAttack(m_data.m_wpn_data->GetWeaponType(), TICKS_TO_TIME(pLocalController->GetTickBase())))
		return;

	const float best_time = best_target.best_record->m_simulation_time;
	...
	best_target.best_record->apply(entity->m_pawn);

	QAngle_t spreadAngle = GetAimPunch(pLocalPawn);
	if (!hitchance(entity->m_pawn, active_weapon, best_target.best_point))
	{
		best_target.best_record->reset(entity->m_pawn);
		return;
	}

	QAngle_t pViewAngles = pUserCmd->pViewAngles->angValue;
	QAngle_t vNewAngles = GetAngularDifferenceRageCurAngle(pUserCmd, best_target.best_point, pLocalPawn, pViewAngles) - spreadAngle;
	QAngle_t best_point_final = NormalizeAnglesRage(pViewAngles + vNewAngles);

	F::ANTIAIM::bestTargetSimTime = best_target.best_record->m_simulation_time;

	pUserCmd->pViewAngles->angValue = best_point_final;

	if (auto active_weapon_base_gun = I::GameResourceService->pGameEntitySystem->Get<C_CSWeaponBaseGun>(active_weapon->GetRefEHandle()))
	{
		AutoScope(pCmd, active_weapon_base_gun);
	}

	if (CanShoot(pLocalController, active_weapon_base_gun, pLocalController->GetTickBase()))
	{
